				   ...
				   struct stm32_pll_dt_cfg *pll)
{
	...
	pll->src = fdt_read_uint32_default(fdt, subnode_pll, "src", UINT32_MAX);
...
static const struct mux_rifsc_cfg tab_mux_rifsc[MUX_NB] = {
	MUX_RIFSC_CFG(MUX_ADC1, STM32MP21_RIFSC_ADC1_ID),
	MUX_RIFSC_CFG(MUX_ADC2, STM32MP21_RIFSC_ADC2_ID),
	MUX_RIFSC_CFG(MUX_USB2PHY1, STM32MP21_RIFSC_USBH_ID),
	MUX_RIFSC_CFG(MUX_USB2PHY2, STM32MP21_RIFSC_USBH_ID),
	MUX_RIFSC_CFG(MUX_DTS, STM32MP21_RIFSC_DTS_ID),
};
...
{
	if (stm32_rcc_has_access_by_id(RCC_RIF_OSCILLATORS))
		return clk_stm32_gate_ready_enable(clk);
...
static TEE_Result clk_stm32_flexgen_rates_array(struct clk *clk,
						size_t start_index,
						unsigned long *rates,
						size_t *nb_elts)
{
	static struct mutex mu = MUTEX_INITIALIZER;
...
static void __maybe_unused clk_stm32_flexgen_pm_restore(struct clk *clk)
{
	TEE_Result ret = TEE_ERROR_GENERIC;
	size_t pidx = 0;
...
static TEE_Result clk_stm32_rif_gate_enable(struct clk *clk)
{
...
static TEE_Result clk_stm32_rif_composite_set_parent(struct clk *clk,
						     size_t pidx)
{
	struct clk_stm32_rif_composite_cfg *cfg = clk->priv;
...
static TEE_Result clk_stm32_rif_composite_set_rate(struct clk *clk,
						   unsigned long rate,
						   unsigned long parent_rate)
{
	struct clk_stm32_rif_composite_cfg *cfg = clk->priv;
...
static TEE_Result clk_stm32_rif_composite_gate_enable(struct clk *clk)
{
	struct clk_stm32_rif_composite_cfg *cfg = clk->priv;
...
static RIF_GATE(ck_icn_p_dbgmcu, &ck_icn_apb3, 0,
...
unsigned long clk_stm32_clock_frequency_calculator(uint32_t ckintsel)
{
	...
	while ((fcalctwc_val >= min_fcalctwc) && !freq_meas_complete_flag) {
		 ...
		 */
		io_clrsetbits32(rcc_base + RCC_FCALCOBS0CFGR,
		 ...
		 */
		io_clrsetbits32(rcc_base + RCC_FCALCCR2,
				SHIFT_U32(0xf, RCC_FCALCCR2_FCALCTWC_SHIFT),
				SHIFT_U32(fcalctwc_val,
					  RCC_FCALCCR2_FCALCTWC_SHIFT));
		 ...
		 */
		io_setbits32(rcc_base + RCC_FCALCCR2,
			     SHIFT_U32(0xc, RCC_FCALCCR2_FCALCTYP_SHIFT));
		/*
		 ...
		 */
		io_setbits32(rcc_base + RCC_FCALCCR2,
			     SHIFT_U32(0x3, RCC_FCALCCR2_FCALCMD_SHIFT));
		...
		if (regval & RCC_FCALCSR_FVAL_OVERFLOW) {
			...
			if (!(regval & RCC_FCALCSR_FVAL_MASK)) {
				...
				denom_val = SHIFT_U32(1, fcalctwc_val + 1) + 5;

				/* Compute measured frequency */
				ckin_freq_val =
					(uint64_t)((regval &
						    RCC_FCALCSR_FVAL_MASK) -
						   16) * (uint64_t)ref_freq;
...
{
	/* Restore clock tree */
	if (stm32mp2_init_clock_tree(&stm32mp21_clock_data,
				     &stm32mp21_clock_pdata))
...
static TEE_Result stm32_rcc_pm(enum pm_op op, unsigned int pm_hint,
			       const struct pm_callback_handle *h __unused)
{
	TEE_Result res = TEE_ERROR_GENERIC;
